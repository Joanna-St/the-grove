...
- Time: in-game day/night cycle visible while playing; no real-time tracking when closed
...
### Forage
- Berries, roots, herbs — used to feed creatures and restore grove areas (alongside heartwood)
- Feeding is an active interaction capped once or twice per in-game day per creature, depending on personality
- Feeding builds bond and triggers a personality-specific response — it is a small moment, not a background action
- No penalty for skipping a feeding; bond only grows through interactions, so a missed feeding is simply a missed opportunity
- A feeding indicator per creature shows current state: available / satisfied / full — timing and expression vary by personality
- Most creatures can also bring forage passively; varies by personality
...
### Glamour
- Magical energy — the fuel for the grove's protective shield
- Accumulates passively and through creature contributions; spent on area unlocks and on tending the Silvanus statue
- Sources: pseudodragon, flumph, pixie (all leak Feywild energy passively)

### Protection
- A separate stat (0–100%) representing the shield generated by the Silvanus statue
- Decays on its own over time — the shield naturally weakens without fuel
- Tending the statue spends glamour to restore protection
- Low protection = slowed passive generation across the grove + visible statue deterioration (cracked stone, dimming light)
- High protection = healthy passive generation + glowing, moss-covered, alive statue
- Protection never causes resource loss — only slows or pauses generation. A few foraging trips and a tend always recovers the situation.
- Decay rate scales with grove size (more areas = bigger shield to maintain) and is highest when the Feywild Boundary is active
...
- Base passive income scales with protection level — a well-maintained shield means a thriving grove
...
- Passive generation never hits zero regardless of protection — a faint trickle always remains

---

## Time System

- In-game day/night cycle while playing — affects creature behaviour, resource availability, atmosphere
- Resources only tick while the game is open — the grove waits, it does not suffer
- No real-time tracking when closed; returning players find everything as they left it
...
### Passive (idle)
- Resources tick in over time (rate: protection level × grove size)
...
- Feed creatures — spend forage, build bond, trigger personality-specific response (once or twice per creature per day)
...
- **Feeding:** Twice a day — expends energy at an alarming rate. Approaches with enormous careful focus, takes food with surprising gentleness, then immediately knocks something over. Indicator resets faster than you'd expect.
...
- **Feeding:** Once a day. Accepts as if it was always owed. Does not look at you while eating. You just suddenly know it found the offering adequate.
...
- **Feeding:** Twice a day. Teleports directly onto the food before you've finished placing it. Reaction is immediate and chaotic. Indicator resets suspiciously fast.
...
- **Feeding:** Once a day. Courteous, unhurried. Glows slightly warmer afterward. If the grove is thriving it pulses with something that feels like contentment.
...
- **Feeding:** Once a day — not feeding in any conventional sense. You leave an offering at a spot it frequents. The wisp drifts through. The forage is gone. You are not sure it noticed. Something shifted anyway.
...
- **Feeding:** Once a day, unpredictable timing. Sometimes takes the food; sometimes relocates it somewhere inexplicable. Bond counts either way.
...
- **Feeding:** Once a day. Food must be left, not offered directly — it will not take anything from your hand yet. You step back; it approaches. Bond builds through the restraint, not the gesture.
...
- **Feeding:** Twice a day. Visibly hopeful about a third time. Grateful in a way that is slightly embarrassing for both of you.
...
- **Ongoing maintenance:** The Feywild Boundary adds the highest protection decay multiplier in the game — the volatile magical pressure on the shield is real. Neglect it and the statue visibly suffers; tend it and the boundary stabilises.
...
- Protection decay rate (base + per-area scaling + Feywild Boundary multiplier)
...
- Config file (all tunable values: resource rates, restoration costs, day length, event frequencies, protection decay rate, dev speed multiplier)
- Save/load system (JSON)
- In-game day/night cycle while playing, dev speed mode
- Resource tracking and protection stat (no art yet — rectangles and text only)
